...
def trim_dataset(orig_data, colindex, valuename):
    ...
    for i in range(len(orig_data)):
        if orig_data[i][colindex] == valuename:
            tempy = orig_data[i]
            temp___row = [x for ci, x in enumerate(tempy) if ci != colindex]
...
def predictFUNCTION(dict, testMatrix, list_of_labels, majority_label):
    classified = []
...
def accuracyMETRIC(classified_list, testMatrix):
    av_accuracy = 0
    sum_acc = 0
    accuracy_list = []
...
def f1METRIC(classified_list, testMatrix):
    TP = 0
    TN = 0
    FP = 0
    FN = 0
    ...
    for i in range(len(testMatrix)):
        if testMatrix[i][mat_col_last_ind] + classified_list[i] == 2:
            TP += 1
        # elif testMatrix[i][mat_col_last_ind] + classified_list[i] == 0:
        elif testMatrix[i][mat_col_last_ind] + classified_list[i] == 0:
            TN += 1
        # elif testMatrix[i][mat_col_last_ind] == 0 and classified_list[i] == 1:
        elif testMatrix[i][mat_col_last_ind] == 0 and classified_list[i] == 1:
            FP += 1
        else:
            FN += 1
...
def create_ID3_tree(Sdata, numAttributes, depth, to_be_visited, majority_label, limiting_depth):
    ...
    if numAttributes <= 1:
        ...
        last_col = np.array([elements_of_column(Sdata, len(Sdata[0]) - 1)])
        numFeatures = len(Sdata[0]) - 1
        k = int(math.ceil(math.log(numFeatures,2)))
        desiredCols = np.array(random.sample(xrange(numFeatures), k))
        k_m_features = Sdata[:, desiredCols]
        k_m_Data = np.concatenate((k_m_features, last_col.T), axis=1)
        best_info_gain, temp_kmA_index = ID3_all_examples(k_m_Data, k)

        temp_A_index = (desiredCols[temp_kmA_index])
        ...
        tree = {A_index: {}}
        len_A_values = len(A_values)
        for val in range(len_A_values):
            temp_depth = copy.deepcopy(depth)
            temp_depth += 1
            temp_to_be_visited = copy.deepcopy(to_be_visited)
            temp_Ddata = copy.deepcopy(Sdata)
            subSdata = np.array(trim_dataset(temp_Ddata, temp_A_index, A_values[val]))
            numAttributes = len(subSdata[0]) - 1
            subtree = create_ID3_tree(subSdata, numAttributes, temp_depth, temp_to_be_visited, majority_label,
                                      limiting_depth)
...
def cross_valid_Experiment(splitPath, limiting_depth, method):
    ...
    for i in range(len(all_filenames)):
        ...
        tree = ID3_implementation(combined_data, numTrainingAttributes, depth_of_tree, to_be_visited, majority_label,
                                  limiting_depth, method)
